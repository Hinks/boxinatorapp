...
public class DatabaseVerticle extends AbstractVerticle {
  ...
  /**
   * Inserts a new box to the database. If the insert was successful
   * this method replies to the sender with a json object containing
   ...
   * @param message Message that is sent to the consumer address this verticle listens to.
   */
  private void saveBox(Message<JsonObject> message) {
    ...
    f1
      .compose(conn -> {
        ...
        conn.updateWithParams(SqlQueries.SAVE_BOX, params, f2);
        return f2;
      })
      .compose(updateResult -> {

        int lastInsertId = updateResult.getKeys().getInteger(0);
        message.reply(new JsonObject().put("id", lastInsertId));
